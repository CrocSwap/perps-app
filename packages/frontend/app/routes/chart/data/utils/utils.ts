...
export const isRepositioningSameLine = (
    selectedLine: LabelLocationData | undefined,
    tempSelectedLine: LabelLocationData | undefined,
    isCompletedReposition: boolean,
) => {
    return (
        isCompletedReposition &&
        selectedLine?.parentLine?.oid &&
        tempSelectedLine?.parentLine?.oid &&
        selectedLine.parentLine.oid === tempSelectedLine.parentLine.oid
    );
};
